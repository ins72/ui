...
const DraftsPage = ({}) => {
  const [draftsProducts, setDraftsProducts] = useState<ProductDraft[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await dataService.getProducts({ status: "draft", limit: 10 });
        if (response.products) {
          setDraftsProducts(response.products);
        }
      } catch (err) {
